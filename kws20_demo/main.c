...
#define THRESHOLD_HIGH 450 //350 // voice detection threshold to find beginning of a keyword (here!!)
#define THRESHOLD_LOW 300 //100 // voice detection threshold to find end of a keyword (here!!)
...
//                                         "Cough", "Unknown"};
const char keywords[NUM_OUTPUTS][10] = { "Up", "Down", "Left", "Right", "Stop", "Go", "Yes", "No",
...
int cough_counter = 0; // cough counter
...
#endif //#ifdef ENABLE_SILENCE_DETECTION
        {
            ...
            if (ai85Counter >= SAMPLE_SIZE) {
                ...
                if (!ret || out_class == NUM_OUTPUTS - 1) {
                    ...
                    if (strcmp(keywords[out_class], "Cough") == 0) {
                        cough_counter++;
                    }
                    
                }
                PR_DEBUG("Number of coughs today: %d\n", cough_counter);
                PR_DEBUG("\n----------------------------------------- \n");
